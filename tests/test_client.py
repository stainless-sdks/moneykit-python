...
from moneykit._models import BaseModel, FinalRequestOptions
from moneykit._base_client import BaseClient, make_request_options
...
class TestMoneykit:
    client = Moneykit(base_url=base_url, api_key=api_key, _strict_response_validation=True)

    def test_raw_response(self) -> None:
        response = self.client.get("/institutions", options={"params": {}}, cast_to=httpx.Response)
...
class TestAsyncMoneykit:
    client = AsyncMoneykit(base_url=base_url, api_key=api_key, _strict_response_validation=True)

    async def test_raw_response(self) -> None:
        response = await self.client.get("/institutions", options={"params": {}}, cast_to=httpx.Response)
        assert response.status_code == 200
        assert isinstance(response, httpx.Response)
